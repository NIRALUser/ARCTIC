...
::ComputeDistributions()
{
  ...
  for (unsigned int iclass = 0; iclass < numClasses; iclass++)
  {
    ...
    for (unsigned int ichan = 0; ichan < numChannels; ichan++)
    {
      ...
      for (ind[2] = 0; ind[2] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[2]); ind[2] += skips[2])
        for (ind[1] = 0; ind[1] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[1]); ind[1] += skips[1])
          for (ind[0] = 0; ind[0] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[0]); ind[0] += skips[0])
          {
            mu += 
      ...
      for (unsigned int c = r; c < numChannels; c++)
      {
        ...
        for (ind[2] = 0; ind[2] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[2]); ind[2] += skips[2])
          for (ind[1] = 0; ind[1] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[1]); ind[1] += skips[1])
            for (ind[0] = 0; ind[0] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[0]); ind[0] += skips[0])
            {
              const double diff1 = LOGP(m_InputImages[r]->GetPixel(ind)) - mu1;
              const double diff2 = LOGP(m_InputImages[c]->GetPixel(ind)) - mu2;
  ...
  for (ind[2] = 0; ind[2] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[2]); ind[2] += skips[2])
    for (ind[1] = 0; ind[1] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[1]); ind[1] += skips[1])
      for (ind[0] = 0; ind[0] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[0]); ind[0] += skips[0])
      {
        ...
        const double diff0 = ind[0] - m_XMu[0];
        m_XStd[0] += diff0*diff0;
        const double diff1 = ind[1] - m_XMu[1];
        m_XStd[1] += diff1*diff1;
        const double diff2 = ind[2] - m_XMu[2];
        m_XStd[2] += diff2*diff2;
      }
        ...
        for (unsigned int order = 0; order <= m_MaxDegree; order++)
          for (unsigned int xorder = 0; xorder <= order; xorder++)
            for (unsigned int yorder = 0; yorder <= (order-xorder); yorder++)
            {
              ...
              const double xc = (ind[0] - m_XMu[0]) / m_XStd[0];
              const double yc = (ind[1] - m_XMu[1]) / m_XStd[1];
              const double zc = (ind[2] - m_XMu[2]) / m_XStd[2];
  ...
  bool fullRes)
{
  ...
  const unsigned int numChannels = inputs.GetSize();
  const unsigned int numClasses = m_Probabilities.GetSize();

  const unsigned int numCoefficients =
    (m_MaxDegree+1) * (m_MaxDegree+2)/2 * (m_MaxDegree+3)/3; /* if m_MaxDegree = 4/3/2, then this is 35/20/10 */

  itkDebugMacro(<< numClasses << " classes\n");
  itkDebugMacro(<< numCoefficients << " coefficients\n");

  itkDebugMacro(<< "Computing inverse covars...\n");
  DynArray<MatrixType> invCovars;                             /* compute inverse matrix for each tissue type */
  for (unsigned int iclass = 0; iclass < numClasses; iclass++)
  ...
  unsigned int numEquations = m_Basis.rows();   /*  since m_Basis is [numEqu * 35 ] matrix */
  ...
  for (unsigned int ichan = 0; ichan < numChannels; ichan++)
  {
    ...
    for (unsigned int jchan = 0; jchan < numChannels; jchan++)
    {
      ...
      for (ind[2] = 0; ind[2] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[2]); ind[2] += sampleofft[2])
        for (ind[1] = 0; ind[1] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[1]); ind[1] += sampleofft[1])
          for (ind[0] = 0; ind[0] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[0]); ind[0] += sampleofft[0])
          {
  ...
  for (unsigned int ichan = 0; ichan < numChannels; ichan++)
  {
    ...
    for (ind[2] = 0; ind[2] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[2]); ind[2] += workingofft[2])
      for (ind[1] = 0; ind[1] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[1]); ind[1] += workingofft[1])
        for (ind[0] = 0; ind[0] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[0]); ind[0] += workingofft[0])
        {
    ...
    for (ind[2] = 0; ind[2] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[2]); ind[2] += workingofft[2])
      for (ind[1] = 0; ind[1] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[1]); ind[1] += workingofft[1])
        for (ind[0] = 0; ind[0] < static_cast<typename ProbabilityImageIndexType::IndexValueType>(size[0]); ind[0] += workingofft[0])
        {
          ...
          const double logd = LOGP(m_InputImages[ichan]->GetPixel(ind)) - logb;
          double d = EXPP(logd);
