...
::CopyInput()
{
  ...
  //TODO:  Replace with the duplicate image filter
  //DEBUG:: This appears to be the same as DuplicateImageFilter.
  ...
  for (unsigned int i = 0; i < m_InputImages.GetSize(); i++)
  {
    ...
    img->SetRegions(m_InputImages[0]->GetLargestPossibleRegion());
    img->Allocate();

    InputImageIndexType ind;
    for (ind[2] = 0; ind[2] < static_cast<typename InputImageIndexType::IndexValueType>(size[2]); ind[2]++)
      for (ind[1] = 0; ind[1] < static_cast<typename InputImageIndexType::IndexValueType>(size[1]); ind[1]++)
        for (ind[0] = 0; ind[0] < static_cast<typename InputImageIndexType::IndexValueType>(size[0]); ind[0]++)
        {
...
::ComputeAverageGradientNorm()
{
  ...
  for (ind[2] = 1; ind[2] < static_cast<typename InputImageIndexType::IndexValueType>(size[2]-1); ind[2]++)
    for (ind[1] = 1; ind[1] < static_cast<typename InputImageIndexType::IndexValueType>(size[1]-1); ind[1]++)
      for (ind[0] = 1; ind[0] < static_cast<typename InputImageIndexType::IndexValueType>(size[0]-1); ind[0]++)
      {
...
::Filter(DynArray<InputImagePointer> images)
{
  ...
  m_DiffusionImage->CopyInformation(m_InputImages[0]);
  m_DiffusionImage->SetRegions(m_InputImages[0]->GetLargestPossibleRegion());
  m_DiffusionImage->Allocate();

  // Initialize to zero
  InternalImageIndexType ind;
  const InputImageRegionType region = m_InputImages[0]->GetLargestPossibleRegion();
  const InternalImageSizeType size = region.GetSize();
  for (ind[2] = 0; ind[2] < static_cast<typename InputImageIndexType::IndexValueType>(size[2]); ind[2]++)
    for (ind[1] = 0; ind[1] < static_cast<typename InputImageIndexType::IndexValueType>(size[1]); ind[1]++)
      for (ind[0] = 0; ind[0] < static_cast<typename InputImageIndexType::IndexValueType>(size[0]); ind[0]++)
